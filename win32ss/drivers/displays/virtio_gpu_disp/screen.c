 ...
 * PURPOSE:     Screen related functions for Virtio Virgl GPU Display Driver
...
IntInitScreenInfo(
   ...
   PDEVINFO pDevInfo)
{
    VirtioPrintDebug("IntInitScreenInfo\n");

    ULONG ModeInfoSize;
    PVIDEO_MODE_INFORMATION ModeInfo, ModeInfoPtr, SelectedMode = NULL;
    ULONG ModeCount = GetAvailableModes(ppdev->hDriver, &ModeInfo, &ModeInfoSize);
    if(ModeCount == 0)
        return FALSE;
...
DrvGetModes(
   ...
   OUT DEVMODEW *pdm)
{
    VirtioPrintDebug("DrvGetModes\n");

    ULONG ModeInfoSize;
    PVIDEO_MODE_INFORMATION ModeInfo, ModeInfoPtr = NULL;
    ULONG ModeCount = GetAvailableModes(hDriver, &ModeInfo, &ModeInfoSize);
    if(ModeCount == 0)
        return 0;
